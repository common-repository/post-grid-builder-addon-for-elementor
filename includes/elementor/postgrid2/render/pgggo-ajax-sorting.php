...
if ($settings['pgggo_enable_sorter'] == 'yes'):
    ...
    if(!empty($pagetaxondataselect) && is_array($pagetaxondataselect) ){
      foreach ($pagetaxondataselect as $keypaga => $valuepage) {
        if(!empty($valuepage) && $valuepage[0] !== "" ){
          $pgggo_selection_ajax_array_sel[] =array(
            'taxonomy' => $keypaga,
             'field' => 'term_id',
             'terms' => $valuepage,
...
if (isset($_POST['pgggopage'])) {
  ...
  $pgggo_args['paged'] = (int)$_POST['pgggopage'];
}


$pgggo_query = new \WP_Query($pgggo_args);
$pgggo_args_pass = wp_json_encode($pgggo_args);
?>


<div data-pgggoajaxsorting='<?php echo $pgggo_args_pass; ?>' data-ppggoargs="{}" data-pgggopagenum="1" id="pgggo-container" class="pgggo-container-ajaxon pgggo-container <?php if($settings['pgggo_enable_ajax_pagination'] == 'yes'){echo 'pgggo-container-ajax'; } if($pgggo_enable_ajax_loadmore == 'yes'){echo ' pgggo-container-ajax-loadmore'; }  ?>">
  ...
  <div id="pgggo-pagination-top" class="pgggo-pagination pgggo-pagination-top" >
              <?php echo paginate_links($pgggo_user_data->pgggo_paginate_args($pgggo_query)); ?>
