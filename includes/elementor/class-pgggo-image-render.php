...
class Pgggo_Featurred_Image_Getter extends Group_Control_Image_Size
{
    ...
    public static function get_attachment_image_src($attachment_id, $image_size_key, array $settings)
    {
        ...
        if ('custom' !== $size) {
            ...
            require_once plugin_dir_path(__FILE__) . 'library/bfi-thump.php';
    ...
    protected function init_fields()
    {
        $fields         = [];
        $fields['size'] = [
            'label'       => esc_html_x('Image Size', 'Image Size Control', 'pgggo'),
            'type'        => Controls_Manager::SELECT,
            'label_block' => false,
        ];
        $fields['custom_dimension'] = [
            'label'       => esc_html_x('Image Dimension', 'Image Size Control', 'pgggo'),
            'type'        => Controls_Manager::IMAGE_DIMENSIONS,
            'description' => esc_html__('You can crop the original image size to any custom size. You can also set a single value for height or width in order to keep the original size ratio.', 'pgggo'),
            'condition'   => [
                'size' => 'custom',
            ],
            'separator'   => 'none',
    ...
    private function get_image_sizes()
    {
        ...
        $image_sizes['full'] = esc_html_x('Full', 'Image Size Control', 'pgggo');
        if (!empty($args['include']['custom']) || !in_array('custom', $args['exclude'])) {
            $image_sizes['custom'] = esc_html_x('Custom', 'Image Size Control', 'pgggo');
        }
        return $image_sizes;
    }
    /**
